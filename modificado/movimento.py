...
def neighbors(location, size=(4, 4)):
  ...
  x, y = (0, 3) if location is None else location
  width, height = size
  #  célula acima
  if y - 1 >= 0:
    yield x, y - 1
...
def bump_wall(location):
  return move_forward(location-1)
...
def neighbor(location, direction, size=(4, 4)):
  ...
  if x + dx < 0 or y + dy < 0 or x + dx > width-1 or y + dy > height -1:
    print("\n")
    print('Você bateu numa parede, não há como seguir por essa direção')
    print("\n")
    bump_wall(location)
